...
public class JwtUtil implements Serializable {
	...
	public UsernamePasswordAuthenticationToken getAuthentication( final String token,
		...
		final JwtParser jwtParser =
				Jwts.parser().setSigningKey( SecurityConstants.SIGNING_KEY );
		...
		final Collection< ? extends GrantedAuthority> authorities = Arrays
				.stream( claims.get( SecurityConstants.AUTHORITIES_KEY ).toString()
						.split( "," ) )
	...
	public Boolean isTokenExpiredBeforeFiveMin( String token ) {
		final Date expiration = getExpirationDateFromToken( token );
		return TimeUnit.MILLISECONDS
				.toMinutes( expiration.getTime() )
				- TimeUnit.MILLISECONDS
						.toMinutes( new Date().getTime() ) <= 5
				&& TimeUnit.MILLISECONDS
						.toMinutes( expiration.getTime() )
						- TimeUnit.MILLISECONDS
								.toMinutes( new Date().getTime() ) > 0;
	}
}
