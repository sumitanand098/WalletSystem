...
public class WalletController {
	...
	public ResponseEntity<WalletResponse> getTotalAmountDetails( @PathVariable int id ) {
		...
		WalletResponse walletResponse = walletResponseWrapper.createResponse( Constants.ResponseConstants.SUCCESS, walletTotalBalance.totalBalance( id ) );
	...
	public ResponseEntity<WalletResponse> getPassbookDetails( @PathVariable int id ) {
		...
		WalletResponseWrapper walletResponseWrapper = new WalletResponseWrapper();
		WalletResponse walletResponse = walletResponseWrapper.createResponse( Constants.ResponseConstants.SUCCESS, passbookDetails.passbookDetails( id ) );
		return new ResponseEntity<WalletResponse>( walletResponse, HttpStatus.OK );
	}
	...
	@PostMapping( path = "/passbook",
			produces = MediaType.APPLICATION_JSON_VALUE )
	public ResponseEntity<WalletResponse> getPassbookRangeDetails( @RequestBody UserPassbook userPassbook ) {
		//		List<WalletPassbookEntity> listWP = passbookDetails.passbookDetails( userDet.getUserId() );
		WalletResponseWrapper walletResponseWrapper = new WalletResponseWrapper();
		WalletResponse walletResponse = walletResponseWrapper.createResponse( Constants.ResponseConstants.SUCCESS, passbookDetails.passbookRangeDetails( userPassbook ) );
		return new ResponseEntity<WalletResponse>( walletResponse, HttpStatus.OK );
	}
	...
	@PostMapping( path = "/walletTransaction",
			...
			produces = MediaType.APPLICATION_JSON_VALUE )
	public ResponseEntity<WalletResponse> debitTransactionDetails( @RequestBody TransactionData transactionData ) {
		WalletResponseWrapper walletResponseWrapper = new WalletResponseWrapper();
		WalletResponse walletResponse = walletResponseWrapper.createResponse( Constants.ResponseConstants.SUCCESS, account.withdraw( transactionData ) );
		return new ResponseEntity<WalletResponse>( walletResponse, HttpStatus.OK );
	}
	...
	@PostMapping( path = "/bankTransaction",
			consumes = MediaType.APPLICATION_JSON_VALUE,
			produces = MediaType.APPLICATION_JSON_VALUE )
	public ResponseEntity<WalletResponse> creditTransactionDetails( @RequestBody TransactionData transactionData ) {
		WalletResponseWrapper wrw = new WalletResponseWrapper();
		WalletResponse walletResponse;
		if ( account.bankBalanceCheck() )
			walletResponse = wrw.createResponse( Constants.ResponseConstants.SUCCESS, account.deposit( transactionData ) );
		else
			walletResponse = wrw.createResponse( Constants.ResponseConstants.SUCCESS, Constants.ResponseConstants.FAILURE );
		return new ResponseEntity<WalletResponse>( walletResponse, HttpStatus.OK );
	}
